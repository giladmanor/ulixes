class GmmController < AdminController
   CLUSTER_NAMES = ["blue", "red", "yellow", "green", "brown", "purple","maroon", "gray", "pink", "crimson", "indigo","lime" ]
  
  def list
    @clusters = @account.clusters.sort{|a,b| b.users.size<=>a.users.size}.take(10).select{|c| c.users.size>2}
    logger.debug(@clusters.each_with_index.map{|c,i| [i,c.users.size]})
    render :json=> @clusters.each_with_index.map{|c,i| {:info=>c, :size=>c.users.size, :color=>CLUSTER_NAMES[i],:name=>CLUSTER_NAMES[i].pluralize.capitalize}} 
  end
  
  def list_users_by
    dim_name = params[:name]
    #logger.debug @account.users.select{|u| !u.vector[dim_name].nil?}.map{|u| u.vector[dim_name]}
    render :json=> @account.users.select{|u| !u.vector[dim_name].nil?}.sort{|a,b| b.vector[dim_name] <=> a.vector[dim_name]}.take(5)
  end
  ...
  def get_cluster
    ...
    users = cluster.users.sort{|a,b| User.distance(a.vector,vector)<=>User.distance(b.vector,vector)}.take(5)
    
    offers = @account.offers.shuffle.take(3)
    
    render :json=> {:info=>cluster,:nodes=>nodes,:users=>users, :offers=> offers,:size=>cluster.users.size}
  end
  
  def get_offers
    offers = @account.offers
    render :json=> {:availables=>[],:confirmed=>offers}
  end
